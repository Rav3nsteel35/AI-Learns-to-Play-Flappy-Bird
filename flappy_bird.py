...
class Pipe:
    ...
    def __init__(self, x):
        ...
        self.top = 0
        self.bottom = 0
        self.PIPE_IMG = pygame.transform.flip(PIPE_IMG, False, True)
    ...
    def set_height(self):
        ...
        self.top = self.height - self.PIPE_IMG.get_height()
    ...
    def draw(self, win):
        win.blit(self.PIPE_IMG, (self.x, self.top))
        win.blit(self.PIPE_BOTTOM_IMG, (self.x, self.bottom))

    def collide(self, bird):
        bird_mask = bird.get_mask()
        top_mask = pygame.mask.from_surface(self.PIPE_IMG)
...
def draw_window(win, birds, pipes, base, score=0, gen=0):
    win.blit(BG_IMG, (0, 0))
...
def main(genomes, config, generation=0):
    nets = []
    ge = []
    birds = []
    ...
    while run:
        ...
        pipe_ind = 0
        if len(birds) > 0:
            if len(pipes) > 1 and birds[0].x > pipes[0].x + pipes[0].PIPE_IMG.get_width():
                pipe_ind = 1
        ...
        for pipe in pipes:
            ...
            if pipe.x + pipe.PIPE_IMG.get_width() < 0:
                rem.append(pipe)
